...
//			}
			foreach($desiredArray as $iteration){
				$blockArray = $this->generateBlockArray($iteration);
		...
		private function generateBlockArray($block_id = 1){
			$rowStart = 1;
			$rows = 100;
			$cols = 100;
			$blocksArray = [];
			...
			/** DEBUG **/
			if($this->debug || $this->debug_lite === true){
				echo 'GENERATOR SETTING : R[' . $rows . '], C[' . $cols . '], RS[' . $rowStart . ']<br />';
			}
			/** DEBUG **/
			...
			for($i=$rowStart;$i<=$rows;$i++) {
				...
				if($this->debug || $this->debug_lite === true) {
					$memory_usage_mb = round((memory_get_usage() / 1000000), 2);
					echo 'Createing of blocks consumes ' . $memory_usage_mb . ' MB usage<br />';
				}
				/** DEBUG **/
			}
			$blockIterator = $block_id;
			...
			for($i=$rowStart;$i<=$rows;$i++){
				for($ii=1;$ii<=$cols;$ii++) {
					...
					/** DEBUG **/
					if($this->debug) {
						echo '<b>got block ' . ($blockIterator) . '(row: ' . $i . ',col: ' . $ii . ') try to search existing links ...</b><br />';
